(() => {
 ...
 let xp = 0, level = 1, soundOn = true, volume = 0.48, audioCtx = null, masterGain = null, toastTimer = null, lastFocused = null;
 ...
 const i18n = {
  fr:{
   ...
   rLead:"Profil orienté Software Engineering, Infrastructure et Cybersécurité, avec une forte appétence pour l'automatisation. Après des expériences chez Vinci Construction Grands Projets et Axens, je rejoins AXA le 28 septembre 2026 comme Software Engineer Analyst Apprentice.",
  ...
  en:{
   ...
   rLead:'Profile focused on Software Engineering, Infrastructure and Cybersecurity, with a strong interest in automation. After experiences at Vinci Construction Grands Projets and Axens, I join AXA on September 28, 2026 as a Software Engineer Analyst Apprentice.',
 ...
 function axaHasStarted(){return new Date() >= new Date('2026-09-28T00:00:00+02:00')}
 ...
 function setLang(next){
  ...
  $('langFr').setAttribute('aria-pressed',String(lang==='fr'));$('langEn').setAttribute('aria-pressed',String(lang==='en'));
  const languageLabel=lang==='fr'?'Passer le site en anglais':'Switch the site to French';$('langToggle').setAttribute('aria-label',languageLabel);$('rLangToggle').setAttribute('aria-label',languageLabel);
 ...
 function showEndScreen(){
  lastFocused=document.activeElement;endShown=true;$('endLevel').textContent=String(level).padStart(2,'0');updateCollectiblesUI();spawnConfetti();$('endScreen').classList.add('show');$('endScreen').setAttribute('aria-hidden','false');$('endContinueBtn').focus();beep(987,.16);setTimeout(()=>beep(1318,.18),130);saveGame(true);
 }
 function closeEndScreen(){$('endScreen').classList.remove('show');$('endScreen').setAttribute('aria-hidden','true');(lastFocused&&document.contains(lastFocused)?lastFocused:canvas).focus()}
 ...
 function openNode(n){
  lastFocused=document.activeElement;$('modalLabel').textContent=n.label;$('modalTitle').textContent=n.title;$('modalBody').innerHTML=n.body;$('modalWrap').classList.add('show');$('modalWrap').setAttribute('aria-hidden','false');$('closeModal').focus();beep(520,.055);
 ...
 function closeModal(){$('modalWrap').classList.remove('show');$('modalWrap').setAttribute('aria-hidden','true');(lastFocused&&document.contains(lastFocused)?lastFocused:canvas).focus()}
 function recruiter(show){if(show)lastFocused=document.activeElement;$('recruiterView').classList.toggle('show',show);$('recruiterView').setAttribute('aria-hidden',String(!show));document.body.style.overflow=show?'hidden':'';if(show){$('closeRecruiter').focus();if(!unlocked.has('recruiter')){unlocked.add('recruiter');addXp(15);checkAchievements();saveGame(true)}}else{(lastFocused&&document.contains(lastFocused)?lastFocused:canvas).focus()}beep(420,.05)}
 function openTerminal(){if($('terminalWrap').classList.contains('show'))return;lastFocused=document.activeElement;$('terminalWrap').classList.add('show');$('terminalWrap').setAttribute('aria-hidden','false');$('terminalOutput').textContent=t().terminalHello;$('terminalInput').value='';setTimeout(()=>$('terminalInput').focus(),20);beep(350,.08)}
 function closeTerminal(){$('terminalWrap').classList.remove('show');$('terminalWrap').setAttribute('aria-hidden','true');(lastFocused&&document.contains(lastFocused)?lastFocused:canvas).focus()}
 ...
 function renderRecruiter(){
  const c=lang==='fr'?{
   ...
   exp:'EXPÉRIENCE',axa:'AXA · Software Engineer Analyst Apprentice',axaD:'28 septembre 2026 · Juin 2027',axaP:'Au sein de Group Data and AI Innovation / Software Engineering : maturité d’ingénierie, bonnes pratiques, sécurité logicielle avec SAST, SCA et DAST, DevSecOps, Open Source et documentation.',
   ...
   exp:'EXPERIENCE',axa:'AXA · Software Engineer Analyst Apprentice',axaD:'September 28, 2026 · June 2027',axaP:'Within Group Data and AI Innovation / Software Engineering: engineering maturity, best practices, software security with SAST, SCA and DAST, DevSecOps, Open Source and documentation.',
 ...
 function bindContactForm(c){
  ...
  form.addEventListener('submit',async e=>{
   ...
   if((data.get('website')||'').toString().trim())return;
    submit.disabled=true;form.setAttribute('aria-busy','true');submit.textContent=c.sending;status.textContent='';beep(520,.035);
   try{
    ...
    finally{submit.disabled=false;form.removeAttribute('aria-busy');submit.textContent=c.send}
  });
 }
 ...
 function showCinematic(){lastFocused=document.activeElement;cinematicIndex=0;cinematicSeen=true;$('cinematic').classList.add('show');$('cinematic').setAttribute('aria-hidden','false');renderCinematic();$('cinematicNext').focus()}
 ...
 $('playBtn').addEventListener('click',()=>{if(soundOn)startMusic();$('startScreen').classList.add('hidden');$('startScreen').setAttribute('aria-hidden','true');running=false;showCinematic();beep(620,.09)});
 $('recruiterBtn').addEventListener('click',()=>recruiter(true));$('startRecruiterBtn').addEventListener('click',()=>{if(soundOn)startMusic();$('startScreen').classList.add('hidden');$('startScreen').setAttribute('aria-hidden','true');running=true;recruiter(true)});$('closeRecruiter').addEventListener('click',()=>recruiter(false));
 ...
 addEventListener('keydown',e=>{
  ...
  if(k==='tab'){
   const overlay=[$('modalWrap'),$('terminalWrap'),$('cinematic'),$('endScreen'),$('recruiterView')].find(el=>el.classList.contains('show'));
   if(overlay){const focusable=[...overlay.querySelectorAll('a[href],button:not([disabled]),input:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])')].filter(el=>el.offsetParent!==null);if(focusable.length){const first=focusable[0],last=focusable[focusable.length-1];if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus()}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus()}}}
  }
